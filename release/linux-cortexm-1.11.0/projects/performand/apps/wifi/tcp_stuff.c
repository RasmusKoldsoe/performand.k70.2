...
pthread_t monitorConnectionStatus_thread;
int done = 0;
bool connected = 0;
...
#define PROVISION_HOSTNAME "DataBoy2.0" ///< Host name of the module for DNS
...
void *monitorConnectionStatus(void* _done) {

	int* done = (int*)_done;
	int out = 0;
	while(!*done) {
		//AtLib_CommandSendNoResponse();
		out = MyAtLib_ReceiveDataHandle();
		if(out) {
			printf("%d\n",out);
			switch(out) {
				case 16: //Client Connected
					gpio_setValue(LED_IND1, GPIO_SET_HIGH);
					connected=1;
					break;
				case 6: //No Client Connected  
					gpio_setValue(LED_IND1, GPIO_SET_LOW);
					connected=0;
					break;
			}
...
int tcp_init(char *port) // "/dev/ttyS3"
{
	...
	//pthread_create(&monitorConnectionStatus_thread, NULL, monitorConnectionStatus, (void *)&done);
#elif __i386__
...
int tcp_send_data(char *data, int len)
{
	...
	bool success = false;
		...
		while(left > 0) {
			...
			//if(!success && AtLib_IsClientConnected()) {
			//	return -1;
				//Reset WIFI connection and restart it!
			//}
