			 ...
			 size_t length, loff_t * offset)
{
	...
	result = i2c_master_send(test_i2c_client, &addr, 1);

	if (result < 0) {
		printk(KERN_WARNING "Write 0: Negative retval: %d\n",result);
		return -1;
	}
	else if (result != 1) {
	...
	result = i2c_master_recv(test_i2c_client, dat, 1);
	if (result < length)
		return -1;

	//if(ret=i2c_master_recv(test_i2c_client, dat, 1)<0)
